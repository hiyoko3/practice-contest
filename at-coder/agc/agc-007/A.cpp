...
int main() {
	...
	P prev = make_pair(0, 0);
	rep(i, H) rep(j, W) {
		string m = n[i].substr(j, 1);
		if (m == ".") continue;
		// cout << "n: " << n[i][j] << endl;

		// r & b
		if ((i+1) < H && (j+1) < W) {
			if (m == n[i+1].substr(j, 1) && m == n[i].substr(j+1, 1)) {
		...
		// l
		if (i > 0) {
			if (m == n[i-1].substr(j, 1) && make_pair(i-1, j) != prev) {
				cout << "Impossible" << endl;
				return 0;
			}
		}
		// t
		if (j > 0) {
			if (m == n[i].substr(j-1, 1) && make_pair(i, j-1) != prev) {
				cout << "Impossible" << endl;
				return 0;
			}
		}

		if (m == "#") prev = make_pair(i, j);
	}
