...
int main() {
	...
	rep(i, M) {
		string k = kit.substr(i, 1);
		k1[k]++;
	}

	int ans = 1;
	for(auto itr: n1) {
		int knum = k1[itr.first];
		if (knum == 0) {
			cout << -1 << endl;
			return 0;
		}

		if (knum >= itr.second) continue;

		if (ans < ceil(itr.second / (knum*1.0))) ans = ceil(itr.second / (knum*1.0));
		// ans = max(ans, (itr.second + knum - 1) / knum);
	}
