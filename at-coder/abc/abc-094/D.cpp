...
int main() {
	...
	ll half = max/2;
	// key: 最小値のidx, val: 最小値
	pair<ll, ll> p = make_pair(max, max);
	...
	rep (i, n) {
		...
		// cout << "i: " << i << ", a: "<< a << " b: " << b << ", " << p.first << " " << p.second << endl;
		if (abs(half - a) < p.second || abs(half - b) < p.second) {
			p.first = i;
			p.second = min({abs(half - a), abs(half - b)});
		}
	}

	cout << max << " " << A[p.first] << endl;
  return 0;
}
