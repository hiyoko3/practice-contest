...
int main() {
	...
	ll x = ((B-1) > N) ? N : B-1;
	ll result = (A*x/B) - A * (x/B);
	cout << result << endl;

	// 解説
	// floor(a/b)は (a-a%b/b)となるので、 ((A*x - A*x%b) / B) - (A * (x - x%B) / B) となる
	// 変形して (A*(x%B) - (A*x%B)) / B
	// = (A*(x%B) - A*(x%B)%B) / B
	// これを floor の式に直すと
	// = floor(A*(x%B) / B)
	// A は定数なので無視して考えると x%B を最大値にすれば良い。そのため、 B-1 が最大値
	// ここで制約として x <= N のため、 N が B-1 以下の場合は N が最大値

	// 別解
	// x = pB + q とおける (p は変数, q = x%B で 0 <= q < B)
	// これを式にあてはめると
	// floor( A(pB+q)/B ) - A * floor( (pB+q)/B)
	// q は十分に無視できる数なので
	// = Ap + floor(Aq / B) - Ap
	// = floor(Aq/B)
  return 0;
