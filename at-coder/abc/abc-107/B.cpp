...
int main() {
	...
	rep(i, H) {
		...
		rep(j, W) {
			if(S[i][j] != white) is_exclude = false;
		}
		rep(j, W) {
			if (m[P(i, j)] == 1) continue;
			m[P(i, j)] = (is_exclude) ? 1 : 0;
		}
	}
	...
	rep(i, W) {
		bool is_exclude = true;
		rep(j, H) {
			if(S[j][i] == white) continue;
			else is_exclude = false;
		}
		rep(j, H) {
			if (m[P(j, i)] == 1) continue;
			m[P(j, i)] = (is_exclude) ? 1 : 0;
		}
	}
