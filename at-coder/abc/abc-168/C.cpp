...
void dfs(int N, int M, int Q, vector<int> A, map<int, vector<int>> m)
{
	...
	{ // 最初に入っている [1] は除いた数列で判定するため
		...
		rep(i, Q)
		{
			...
			int d = m[i][3];
			if ((A[b] - A[a]) == c)
				temp += d; // Aは 1番目から参照しているため、添字は -1 しなくて良い
		}
...
int main()
{
  ...
  const long double PI = acos(-1);
	double h = H * 30 + M * (double)360 / (12 * 60); // 360 / 12 * 60
	double m = M * 6;
	double x = abs(h - m) ; // 度数×(π/180)

	result = sqrt(A*A + B*B - 2 * A * B * cos(x * (PI/180)));
